...
struct StaticPrivateDerived : private StaticPrivateBase
{
    double extra = 0.0;
};

CMM_USE_STATIC_REGISTRY(
    (cmm::detail::make_single_base_static_registry<^^StaticPrivateDerived>()))

int main()
{
    constexpr std::meta::info base_refl =
        cmm::detail::SingleBaseStaticMetadata<^^StaticPrivateDerived>::reflected_base;

    const cmm::info derived_id = cmm::detail::hash_entity(^^StaticPrivateDerived);
    const cmm::info base_spec_id = cmm::detail::hash_entity(base_refl);
    const cmm::info base_type_id = cmm::detail::hash_entity(^^StaticPrivateBase);

    if (!cmm::is_base(base_spec_id)) return 1;
    if (!cmm::is_private_base(base_spec_id)) return 2;
    if (cmm::is_public_base(base_spec_id)) return 3;
    if (cmm::is_protected_base(base_spec_id)) return 4;

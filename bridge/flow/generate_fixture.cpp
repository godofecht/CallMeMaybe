...
int main()
{
    const cmm::flow::GenerationResult generated =
        cmm::flow::generate_wrapper_fragment<^^cmm_e2e_add, ^^cmm_e2e_scale, ^^cmm_e2e_not, ^^cmm_e2e_echo, ^^cmm_e2e_byte_sum, ^^cmm_e2e_echo_bytes>();
    ...
    const std::string echo_bytes_name = cmm::flow::wrapper_name(cmm::get_id<^^cmm_e2e_echo_bytes>());
    ...
    std::cout << "    let echo_bytes_result = " << echo_bytes_name << "(bytes)\n";
    std::cout << "    if echo_bytes_result.error != 0 { return 40 + echo_bytes_result.error }\n";
    std::cout << "    if echo_bytes_result.value.len != 4 { return 17 }\n";
    std::cout << "    if echo_bytes_result.value[0] != 1 or echo_bytes_result.value[3] != 4 { return 18 }\n";
    std::cout << "    return 0\n";
    std::cout << "}\n";
    return 0;
}
